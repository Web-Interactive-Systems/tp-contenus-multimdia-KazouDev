...
type: NoteCard
createdAt: null
viewedAt: 2026-03-17T13:10:54.814Z
---

# Unity XR — lab1 - Initial XR Project Setup
## Unit 1: Creating a Unity URP Project: You can call it MyAirPlaneGameStarter

1.  Create a new project using Unity Hub
2.  Select URP 3D (Universal Render Pipeline) and click create
...
4.  Check the checkbox for the AR plugin you want to use. For example, for Android, select **ARCore** (for iOS, it would be ARKit)
5.  Click “Yes“ if asked “*Do you want to enable the backends? Doing so will RESTART the editor.*”
6.  Select XR Plug-in Manager
7.  Select Project Validation
8.  Potential errors will appear in **red**. If so, click the “Fix all” button (on the top right of the popup window)
...
1.  In the Project window, locate the folder that contains the URP renderer files. This is usually the *Assets/Settings/* folder
2.  In the **step 1 folder,** select each file that ends with **-Renderer**
...
7.  Add a point cloud manager to the XR Origin object by clicking **Add Component** in the Inspector window. Then, enter “ar point” in the search field and select **AR Point Cloud Manager** (this detects **feature Points**: These are distinct points in the physical environment that the AR system can recognize and track. They are typically points with high contrast or distinct texture, making them easier to identify from different angles.)
